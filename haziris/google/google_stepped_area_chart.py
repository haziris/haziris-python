...
GOOGLE_STEPPED_AREA_CHART_HTML = """
<!DOCTYPE html>
<html lang="en">
...
<head>
  ...
  <title>Haziris: Google Stepped Area Chart</title>
</head>
...
<body>
 <div id='chart-div'></div>
  <script>
    function googleSteppedAreaChart( df, options ){
      ...
      function chart(){
        let chart = new google.visualization.SteppedAreaChart(document.getElementById('chart-div'));

        chart.draw( google.visualization.arrayToDataTable( df ), options );
      }
  }
  googleSteppedAreaChart(
    //haziris_data//
  )
  </script>
 </div>
...
def google_stepped_area_chart( df, out_file, options=None ):
  ...
  with open(out_file, 'w') as f:
    ...
    f.write( GOOGLE_STEPPED_AREA_CHART_HTML.replace('//haziris_data//', haziris_data ) )
